...
def start():
    ...
    while True:
        ...
        match select:
            ...
            case 6:
                key_word = view.view_input(text.input_change)
                result = pb.search(key_word)
                if result:
                    if len(result) != 1:
                        view.show_contacts(result, '')
                        current_id = view.view_input(text.input_index)
                    else:
                        current_id = result[0].get('id')
                    new_contact = view.add_contact(text.change_contact)
                    name = pb.change(new_contact, current_id)
